    ...
    def rco_stmts(stmts: List[stmt]) -> List[stmt]:
        ...
        for stmt in stmts:
            ...
            match stmt:
                ...
                case While(test, body):
                    test_bindings = {}
                    new_test = rco_exp(test, False, test_bindings)

                    test_stmts = [Assign([x], e) for x, e in test_bindings.items()]
                    new_test = Begin(test_stmts, new_test)

                    new_body_stmts = rco_stmts(body)

                    new_stmt = While(new_test, new_body_stmts)
...
def explicate_control(prog: Module) -> cif.CProgram:
    ...
    def explicate_if(current_label: str,
                     new_cond: cif.Exp,
                     then_label: str,
                     else_label: str):
        match new_cond:
        ...
        for s in stmts:
            match s:
                ...
                case If(cond, then_stmts, else_stmts):
                    ...
                    explicate_if(current_label, new_cond, then_label, else_label)
                ...
                case While(Begin(test_stmts, test), body):
                    # create labels for the test, loop body, and continuation
                    test_label = gensym('label')
                    body_label = gensym('label')
                    new_cont_label = gensym('label')

                    # jump to the test to kick off the loop
                    add_stmt(current_label, cif.Goto(test_label))

                    # compile the test
                    new_test = explicate_exp(test)

                    if test_stmts:
                        # if there is "prep" to do, put the prep in the test label
                        # then jump to the "condition" label (test_cond_label)
                        test_cond_label = gensym('label')
                        explicate_stmts(test_stmts, test_label, test_cond_label)
                        explicate_if(test_cond_label, new_test, body_label, new_cont_label)
                    else:
                        # if there is no prep to do, just put the test directly in the test label
                        explicate_if(test_label, new_test, body_label, new_cont_label)

                    # compile the loop body
                    # when the body is done running, run the test again
                    explicate_stmts(body, body_label, test_label)
        ...
        for s in stmts:
            match s:
                ...
                case cif.Assign(x, cif.BinOp(e1, op, e2)):
                    instr_name = binop_instrs[name_of(op)]
                    instrs += [x86.NamedInstr('movq', [si_atm(e1), x86.Var(x)]),
...
def uncover_live(program: x86.Program) -> Tuple[x86.Program, Dict[str, List[Set[x86.Var]]]]:
    ...
    def reads_of(i: x86.Instr) -> Set[x86.Var]:
        match i:
            ...
            case x86.Jmp(label) | x86.JmpIf(_, label):
                ...
                # this might be an under-approximation, but that's ok!
    ...
    # run the liveness analysis repeatedly until we reach a fixpoint
    def ul_fixpoint(labels: List[str]):
        ...
        while not fixpoint_reached:
            ...
            for label in labels:
                ul_block(label)
                new_live_befores = live_before_sets[label]
